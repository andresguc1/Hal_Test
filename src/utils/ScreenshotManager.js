...
class ScreenshotManager {
    ...
    async initDB() {
        return new Promise((resolve, reject) => {
            ...
            request.onerror = () => {
                logger.error('Failed to open IndexedDB', request.error, 'ScreenshotManager');
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains(STORE_NAME)) {
                    ...
                    logger.debug('Created screenshots object store', null, 'ScreenshotManager');
                }
            };
        });
    }
   ...
   */
    base64ToBlob(base64, contentType = 'image/png') {
        ...
        if (typeof base64 === 'object' && base64.type === 'Buffer' && Array.isArray(base64.data)) {
        ...
        if (typeof base64 !== 'string') {
            logger.error('Invalid base64 input type', { type: typeof base64, value: base64 }, 'ScreenshotManager');
            throw new Error(`Invalid base64 input: received ${typeof base64} instead of string`);
        }
    ...
    async saveToIndexedDB(nodeId, timing, blob) {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                logger.debug('Screenshot saved to IndexedDB', { nodeId, timing }, 'ScreenshotManager');
                resolve();
            };

            request.onerror = () => {
                logger.error('Failed to save screenshot', request.error, 'ScreenshotManager');
                reject(request.error);
            };
        });
    }
    ...
    async loadFromIndexedDB(nodeId, timing) {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                if (request.result) {
                    logger.debug('Screenshot loaded from IndexedDB', { nodeId, timing }, 'ScreenshotManager');
            ...
            request.onerror = () => {
                logger.error('Failed to load screenshot', request.error, 'ScreenshotManager');
                reject(request.error);
            };
        });
    }
    ...
    async loadScreenshot(nodeId, timing) {
        try {
            ...
            if (cached) {
                logger.debug('Screenshot loaded from cache', { nodeId, timing }, 'ScreenshotManager');
                return cached;
            }
    ...
    async deleteScreenshot(nodeId, timing) {
        try {
            ...
            if (cachedUrl) {
                ...
                logger.debug('Blob URL revoked', { nodeId, timing }, 'ScreenshotManager');
            }

            // 2. Delete from IndexedDB
            await this.deleteFromIndexedDB(nodeId, timing);

            logger.debug('Screenshot deleted', { nodeId, timing }, 'ScreenshotManager');
        } catch (error) {
    ...
    async deleteFromIndexedDB(nodeId, timing) {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                logger.debug('Screenshot deleted from IndexedDB', { nodeId, timing }, 'ScreenshotManager');
                resolve();
            };

            request.onerror = () => {
                logger.error('Failed to delete from IndexedDB', request.error, 'ScreenshotManager');
                reject(request.error);
            };
        });
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = (event) => {
                ...
                if (cursor) {
                    ...
                    logger.debug('Screenshots deleted from IndexedDB', { nodeId }, 'ScreenshotManager');
                    resolve();
                }
            };

            request.onerror = () => {
                logger.error('Failed to delete screenshots', request.error, 'ScreenshotManager');
                reject(request.error);
            };
        });
    }
    ...
    async clearAll() {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onerror = () => {
                logger.error('Failed to clear screenshots', request.error, 'ScreenshotManager');
                reject(request.error);
            };
        });
    }
